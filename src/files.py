...
def map_file_to_date(directory: Path):
    ...
    return directory.name, creation_time


def list_files_in_directory(directory: Path) -> list:
    dir_insides = os.listdir(path=directory)
    dir_files = []
    for file in dir_insides:
...
def list_directories_in_directory(directory: Path) -> list:
    dir_insides = os.listdir(path=directory)
    dir_dirs = [element for element in dir_insides if (directory / element).is_dir()]
    return dir_dirs
