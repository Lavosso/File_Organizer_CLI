...
from mypy.exportjson import convert_instance
...
def delete_mapping(extension_to_delete: str):
    with open("src/file_types.json", "r") as config_file:
        config = json.load(config_file)
    new_config = {extension: file_type for extension, file_type in config.items() if extension != extension_to_delete}
    with open("src/file_types.json", "w") as config_file:
...
def configure_cli():
    ...
    while True:
        decision = input("Would you like to: \n"
                         ...
                         "(x) exit the program?\n")
        if decision == "e":
            extension = input("What file extension would you like to map differently?\n")
            file_type = input("What file type would you like the extension to be mapped to?\n")
